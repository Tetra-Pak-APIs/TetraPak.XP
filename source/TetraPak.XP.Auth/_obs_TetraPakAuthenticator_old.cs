...
// #endif
//
// // [assembly: Dependency(typeof(TetraPakAuthenticator))] obsolete
...
//         readonly TaskCompletionSource<Outcome<Uri>> _authCodeTcs = new();
...
//
//             var waitFor = Config.RedirectUri!.AbsoluteUri; // SystemBrowser.BuildLoopbackUri(Config.RedirectUri!.AbsoluteUri); 
//             using var browser = Config.Browser; // new SystemBrowser(loopbackUri);
//             var target = new Uri(authorizationUri);
//             var result = await browser.InteractAsync(target, waitFor, CancellationToken.None);
//             
//             await authAppDelegate.OpenInDefaultBrowserAsync(new Uri(authorizationUri), Config.RedirectUri);
