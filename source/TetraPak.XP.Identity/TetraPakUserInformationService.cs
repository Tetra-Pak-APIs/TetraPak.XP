    ...
    sealed class TetraPakUserInformationService : IUserInformationService
    {
            ...
            LogMessageId? messageId = null)
        {
            if (grant.AccessToken is null)
                return Outcome<UserInformation>.Fail(
                    new InvalidOperationException($"Grant.{nameof(Grant.AccessToken)} cannot be unassigned"));
                ...
                using (_log?.Section("[GET USER INFO BEGIN]"))
                {
                    try
                    {
                        ...
                        if (!response.IsSuccessStatusCode)
                            tcs.SetResult(Outcome<UserInformation>.Fail(
                                new HttpServerException(response, "Failed when downloading user information")));
