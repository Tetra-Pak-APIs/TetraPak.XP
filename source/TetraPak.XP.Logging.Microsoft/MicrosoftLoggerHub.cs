...
namespace TetraPak.XP.Logging.Microsoft
{
    sealed class MicrosoftLoggerHub : LogBase, ILogger
    {
        ...
        {
            var args = new LogEventArgs(null, logLevel.ToLogRank(), formatter(state, exception), exception,
        ...
        public MicrosoftLoggerHub(ILogger logger, ILog? log, IConfiguration? configuration,
            LogFormatOptions? formatOptions)
            : base(configuration)
        {
            _logger = logger;
            _log = log;
