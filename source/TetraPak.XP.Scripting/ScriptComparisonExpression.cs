...
public sealed class ScriptComparisonExpression : ScriptExpression
{
    ...
    public override async Task<Outcome> RunAsync(IScriptContext? context = null)
    {
        ...
        switch (Operation)
        {
            ...
            case ComparativeOperation.GreaterThan:
                return await LeftOperand.GreaterThan(RightOperand) ? Outcome.Success() : Outcome.Fail("not greater");
    ...
    internal ScriptComparisonExpression(string stringValue, ScriptValue leftOperand, ScriptValue rightOperand, ComparativeOperation operation)
    : base(stringValue)
    {
