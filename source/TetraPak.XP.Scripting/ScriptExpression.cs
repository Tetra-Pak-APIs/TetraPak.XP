...
public abstract class ScriptExpression : StringValueBase
{
    ...
    public static async Task<Outcome<ScriptExpression>> ParseAsync(string stringValue, IScriptContext? context = null)
    {
    ...
    static async Task<Outcome<ScriptComparisonExpression>> parseComparisonExpressionAsync(string stringValue)
    {
        ...
        for (var i = 0; i < parsers.Length; i++)
        {
            ...
            if (!await leftValue.IsValidOperationAsync(operation.Value) || !await rightValue.IsValidOperationAsync(operation.Value.FromRightOperandPerspective()))
        ...
        return Outcome<ScriptComparisonExpression>.Success(new ScriptComparisonExpression(stringValue, leftValue, rightValue, operation.Value));
    }
    ...
    static async Task<Outcome<EatGroupExpressionInfo>> tryEatGroupExpressionAsync(char[] ca, int idx, string stringValue)
    {
        var c = ca[idx];
        string? opToken = null;
        ...
        if (isInverted)
        {
            ...
            if (expression.IsComplete)
                return Outcome<EatGroupExpressionInfo>.Success(new EatGroupExpressionInfo(invertedExpression, idx, opToken!));
        ...
        ++idx;
        if (!tryEatGroup(ca, ref idx, out groupedExpression))
            return Outcome<EatGroupExpressionInfo>.Fail(new FormatException($"Expected group balanced suffix at {idx}"));
            // return Outcome<ScriptLogicExpression>.Fail(new FormatException($"Expected group balanced suffix at {idx}"));
        
        groupOutcome = await parseLogicExpressionAsync(groupedExpression);
        eatToTokens(ca, ref idx, out _, out opToken, ScriptTokens.AndOp, ScriptTokens.OrOp);
        return !groupOutcome 
            ? Outcome<EatGroupExpressionInfo>.Success(new EatGroupExpressionInfo(groupOutcome.Value!, idx, opToken!)) 
